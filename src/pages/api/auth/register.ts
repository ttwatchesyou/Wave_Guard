...
export default async function handler(
  ...
  const { email, password, studentId, firstname, lastname, nickname, age } =
    req.body;
  ...
  try {
    ...
    if (prefix === currentPrefix) {
      ...
      return res
        .status(403)
        .json({ message: "คุณได้เข้าร่วมกิจกรรมไปแล้วในปีก่อน" });
    } else {
      return res
        .status(403)
        .json({ message: "ไม่สามารถระบุรุ่นได้จากรหัสนี้" });
    }
    ...
    return res
      .status(201)
      .json({ message: "สมัครสมาชิกสำเร็จ", role, userId: newUser._id });
  } catch (err) {
